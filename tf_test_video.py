...
def single_pose():
    ...
    while cv2.waitKey(1) < 0:
        ...
        image_width = cv2_image.shape[1]
        image_height = cv2_image.shape[0]
        target_key_points, target_key_points_2 = get_predict_point(keypoints, image_height, image_width)

        # i = 0
        # for joint in KEYPOINT_DICT.keys():
        #     y = target_key_points[joint][0]
        #     x = target_key_points[joint][1]
        #     cv2.circle(cv2_image, (int(x), int(y)), 1, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)
...
def get_predict_point(keypoints, image_height, image_width):
    target_key_points = {}
    target_key_points_2 = {}
    for joint in KEYPOINT_DICT.keys():
        target_key_points[joint] = [
